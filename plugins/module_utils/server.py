...
from ansible_collections.os_migrate.os_migrate.plugins.module_utils.server_port import (
    server_ports,
    ServerPort,
)
...
class Server(resource.Resource):
    ...
    def create(self, conn, block_device_mapping):
        ...
        if not self.migration_params()["data_copy"]:
            ...
            self.update_sdk_params_block_device_mapping_copy(
    ...
    def sdk_params(self, conn):
        ...
        sdk_params["security_groups"] = list(
            map(
    ...
    def update_sdk_params_block_device_mapping_copy(
        self, sdk_params, block_device_mapping
    ):
        params, info = self.params_and_info()
        migration_params = self.migration_params()
        ...
        has_boot_volume = (
            len(
                list(
                    filter(
                        lambda mapping: str(mapping["boot_index"]) != "-1",
            ...
            try:
                if port_creation_mode == "nova":
                    ...
                    raise exc.Unsupported(
                        "Port creation mode specified is unsupported."
                    )
    ...
    def _refs_from_sdk(conn, sdk_res):
        ...
        refs["security_group_ids"] = [sec_group["id"] for sec_group in sec_groups]
        refs["security_group_refs"] = [
    ...
    def _refs_from_ser(self, conn):
        ...
        refs["flavor_id"] = reference.flavor_id(conn, params["flavor_ref"])

        refs["image_ref"] = params["image_ref"]
        if (
            self.migration_params()["data_copy"]
            and self.migration_params()["boot_disk_copy"]
        ):
            refs["image_id"] = None
        else:
